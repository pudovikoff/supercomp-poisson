...
| Тип Запуска | Итераций | Время (с) | $\|w\|_E$ | $\|w\|_C$ | GPU инициализация | apply_A | apply_D_inv | GPU обновление | GPU->CPU копирования | CPU->GPU копирования | MPI обмен | MPI allreduce | CPU редукции | Коэфф. инициализация | OpenMP редукция |
|-------------|----------|-----------|------------|------------|-------------------|--------------|------------------|-------------------|---------------------|---------------------|-----------|---------------|-------------|---------------------|-------------------|
...
Запуск на 2ух GPU оказался намного лучше, чем на 1ом скорее всего из-за загруженности сервера и случайности . 
