...
- Так как нельзя пользоваться shared memory, то пользовался глобальной памятью, редукцией на CPU.
...
| 2000 × 3200, 1 GPU | 2641 | 23.96 | 1.438e-01 | 1.494e-01 | 0.014 | 2.84 | 2.21 | 18.59 | 0.043 | 0.012 | - | - | - | 1.71 | - |
| 2000 × 3200, 2 GPU | 2641 | 18.35 | 1.438e-01 | 1.494e-01 | 0.009 | 1.36 | 0.97 | 14.64 | 0.286 | 0.114 | 0.26 | 0.014 | - | 0.91 | - |
...
При этом видим, что ускорение GPU операций всего 1.5 раза при увеличении количества GPU в 2 раза.

Самая долгая операция - это обновление вспомогательных векторов, которое использует много блокирующих операций, что приводит к задержке.
...
Помимо этого ```dot_product_gpu_ptr``` c редукцией перешла на GPU, что дало итоговый прирост и снижение количества ненужных копирований. 
