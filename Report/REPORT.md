...
Основная сложность заключалась в организации редукции, так как по мотивам задачи нам запрещено использовать разделяемую память,
...
После этого я использовал оптимизацию - подсчёт частичных сумм сразу же на GPU, таким образом я стал копировать массивы намного меньшего размера и получил значительное ускорение для редукции и для всей задачи в целом.

Технические вопросы:
- Расширил поддержку Makefile для CUDA, использовать с компилятором HOST_COMP=mpicxx.

## 7.1 Время исполнения отдельных частей программы

| Operation             | Time (s)    |
|-----------------------|-------------|
| GPU initialization    | 0.811927    |
| apply_A kernels       | 2.919868    |
| apply_D_inv kernels   | 1.592035    |
| Vector ops kernels    | 1.632668    |
| GPU->CPU copies       | 13.883766   |
| CPU->GPU copies       | 9.763282    |
| MPI exchange          | 35.229684   |
| MPI allreduce         | 33.443364   |
| CPU reductions        | 0.066149    |

## 7.2 Сравнение работы программы на большой сетке.
...
| Тип Запуска               | Итераций | Время (с) | $\\|w\\|_E$ | $\\|w\\|_C$ |
|---------------------------|----------|-----------|-------------|-------------|
| 2000 × 3200, последовательный  | 2618       | 0.006733  | 1.441e-01   | 1.491e-01   |
| 2000 × 3200, OpenMp         | 2618       | 123.880203  | 1.327e-01   | 1.412e-01   |
| 2000 × 3200, 20 MPI         | 10289       | 45.816858  | 1.327e-01   | 1.412e-01   |
| 2000 × 3200, 10 MPI × 16 threads  | 10289       | 0.005007  | 1.327e-01   | 1.412e-01   |     
| 2000 × 3200, 2 GPU        | 10289       | 43.237570  | 1.376e-01   | 1.413e-01   |  
