...
### 4.2 Сравнение запусков на сетке 40 × 40  
...
Можем сделать вывод, что время работы на последовательном запуске оказалось быстрее,
потому что не надо было создавать новые нити и копировать часть данных. При этом ошибка решения сопоставимая.
...
## 7. Таблицы для отчета

Таблица 1: OpenMP версии

| Кол-во нитей | Сетка (M×N) | Итераций | Время, с | Ускорение, раз |
|---:|---:|---:|---:|---:|
| 1 | 400×600 | 533 | 10.59| - |
...
![Графики ускорения и времени выполнения](results/speedup_graph.png)

#### График эффективности параллелизма

![График эффективности параллелизма](results/efficiency_graph.png)

Из графиков видно:
- **Ускорение** растет с увеличением числа нитей, достигая 18.56× на 32 нитях для сетки 800×1200
- **Эффективность** снижается с ростом числа нитей (закон Амдала), но остается приемлемой
- Для сетки 400×600 лучшее соотношение цена/качество достигается при 8-16 нитях
